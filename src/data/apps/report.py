...
def add_app_table_entry(app):
    ...
    doc += "<tr>"
    # Icon
    doc += '<td><img src="%s" width="75" height="75"></td>' % app["ios_icon"].replace(
        "{w}x{h}{c}.{f}", "100x100w.png"
    )
...
for app in data["apps"]:
    for other_app in data["apps"]:
        if (
            app["ios_id"] != other_app["ios_id"]
            and app["android_id"] is not None
            and app["android_id"] == other_app["android_id"]
        ):
