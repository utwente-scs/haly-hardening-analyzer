...
if exists(join(dirname(__file__), "scrapingant-key.txt")):
    ...
    print(
        "[!] Please create a scrapingant-key.txt file with a ScrapingAnt API key. You can create one for free at https://scrapingant.com/"
    )
    exit(1)
...
def scrapingant_get(url, response_is_json=True):
    response = requests.get(
        ...
        + quote(url)
        + "&return_page_source=true&x-api-key="
        + api_key,
        headers=headers,
    ).text
        ...
        for key, value in app_data.items():
            ...
            for app in value["d"]["apps"][0]["data"]:
                ...
                apps[app["id"]] = {
                    ...
                    "ios_min_os": (
                        platform_attrs["minimumMacOSVersion"]
                        if "minimumMacOSVersion" in platform_attrs
                        else None
                    ),
                    "ios_icon": platform_attrs["artwork"]["url"],
                    "ios_publisher": attrs["artistName"],
                    "ios_rank": (
                        attrs["chartPositions"]["appStore"]["position"]
                        if name != "Games"
                        else i
                    ),
                    "ios_category": (
                        attrs["chartPositions"]["appStore"]["genreName"]
                        if name != "Games"
                        else "Games"
                    ),
                }
...
for app in data["apps"]:
    ...
    ios_id = app["ios_id"]
    response = requests.get(
        f"https://www.appranking.com/app-profile/{ios_id}/US/as", headers=headers
    ).text
...
api_hash = response.split("/_buildManifest.js")[0].split('<script src="/_next/static/')[
    -1
]
    ...
    for app in data["apps"]:
        ...
        if app["ios_id"] in alternativeto_apps:
            ...
            try:
                ...
                print(
                    "[!] Failed to get info from play store for",
                    alternativeto_apps[app["ios_id"]],
                )
                app["android_id"] = None
...
for app in data["apps"][start:]:
    ...
    print("\n" * 100)  # Clear screen
    print(
        f"Searching for \033[1m{app['ios_name']} - {app['ios_publisher']}\033[0m ({app['ios_bundle_id']}) on Play Store"
    )
    response = requests.get(
        "https://play.google.com/store/search?c=apps&q=" + quote(query)
    ).text
    query_results = []
    for result in response.split("/store/apps/details?id=")[1:]:
        try:
            title_class = [item for item in ["DdYX5", "vWM94c"] if item in result][0]
            publisher_class = [item for item in ["wMUdtb", "LbQbAe"] if item in result][
                0
            ]
            package_id = result.split('"')[0]
            title = (
                result.split(f'class="{title_class}">')[1]
                .split("</")[0]
                .replace("&amp;", "&")
                .replace("&#39;", "'")
            )
            publisher = (
                result.split(f'class="{publisher_class}">')[1]
                .split("</")[0]
                .replace("&amp;", "&")
                .replace("&#39;", "'")
            )
    ...
    for i, result in enumerate(query_results):
        if (
            (result[1] == app["ios_name"] and result[2] == app["ios_publisher"])
            or result[0].lower() in app["ios_bundle_id"].lower()
            or app["ios_bundle_id"].lower() in result[0].lower()
        ):
            print("!", end="")
        print(f"{i+1}. \033[1m{result[1]} - {result[2]}\033[0m ({result[0]})")

    correct_result = input(
        f'Please enter the number of the app matching \033[1m{result[1]}{app["ios_name"]} - {app["ios_publisher"]}\033[0m ({app["ios_bundle_id"]}) (enter 0 if no match, press ENTER for match 1): '
    )
    if correct_result == "0":
        with open(apps_file, "w") as f:
            json.dump(data, f)
        ...
        try:
            ...
            print(
                "[!] Failed to get info from play store for",
                query_results[correct_result][0],
            )
            app["android_id"] = None
