...
class NativeBinary:
            ...
            while True:
                try:
                    if os.name == "nt":
                        ...
                        self._radare = radare2.open_with_timeout_and_memlimit(
                            Config().radare_server,
                            self.path,
                            timeout=Config().radare_timeout,
                            mem_limit_gb=Config().radare_memory_limit,
                            flags=["-2"],
                        )
                    break
    ...
    def extract_strings(self) -> bool:
        ...
        def _extract_strings():
            ...
            strings = [
                s
                for s in strings
                if len([c for c in s if ord(c) < 128]) > 0.9 * len(s)
                and len(set(c for c in s if not c.isalnum())) <= 0.5 * len(s)
            ]
            strings = "\n".join(strings)
        ...
        def _analyze_r2():
            for detector in Detectors():
                try:
                    ...
                    logger.error(
                        f"Error running static analysis for {detector.get_id()} on {self.path}: {e}"
                    )
                    traceback.print_exc()
    ...
    def exec_r2_cmd(self, cmd: str) -> any:
        ...
        if self._radare is None:
            raise Exception(
                "Radare is not initialized, this function can only be used during an analyze() call"
            )
        ...
        for result in results:
            if result["string"] == fun:
                if result["section"].endswith("__objc_methname"):
                    ...
                    logger.debug(
                        f"Found Obj-C function {fun} outside methname section: {result['section']}"
                    )

        return self._find_reference(addresses, False)

    def _find_reference(
        self, addr: str | list[str], track_args: bool = True
    ) -> list[dict]:
        """
        ...
        for a in addr:
            ...
            for result in references:
                ...
                syscalls.append({"offset": result["from"], "args": args})
    ...
    def get_registry_at(self, address: int, lookback: int = 5) -> dict:
        ...
        if instructions is None:
            return registry
        instructions = [
            result["disasm"] for result in instructions if "disasm" in result
        ]
