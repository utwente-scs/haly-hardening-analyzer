...
class App:
    ...
    def get_main_binary_path(self) -> str:
        ...
        """
        return inc.util.workdir_path(join('binary', self.os, self.package_id, f'base.{self.get_binary_extension()}'))
    ...
    def get_native_files(self) -> list[str]:
        ...
        if self.os == 'android':
            # Find so files
            files = glob.glob(join(self.get_decompiled_path(), '*/lib', inc.config.Config().android_abi, '*.so'))
        elif self.os == 'ios':
            # Find mach-o files
            files = inc.util.glob_by_magic(self.get_decompiled_path(), [b'\xFE\xED\xFA\xCE', b'\xFE\xED\xFA\xCF', b'\xCE\xFA\xED\xFE', b'\xCF\xFA\xED\xFE'])
        else:
            raise ValueError(f'Unknown os {self.os}')
        return list(files)
