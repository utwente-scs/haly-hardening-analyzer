...
def _decrypt(app: App) -> bool:
        ...
        if message['type'] == 'send':
            ...
            if message['type'] == 'dump':
                if message['complete']:
                    ...
                    logger.debug(f"Module {message['module']} dumped with size {len(data)} at offset {message['offset']}")
                    ...
                    index_of_app = [i for i, x in enumerate(path) if x.endswith('.app')][0]
                    path = join(app.get_main_binary_path()[:-4], 'Payload', '/'.join(path[index_of_app:]))
    ...
    attempt = 0
    while attempt < 3:
        frida_app = FridaApplication(app, {'context': context}, on_message, on_instrument, Config().dump_timeout, False)
        frida_app.run()
...
def _clean_files(app: App):
    ...
    for root, dirs, files in os.walk(app.get_binaries_path()):
        for file in files:
            imgs = ('.jpg', '.jpeg', '.png', '.gif', '.bmp', '.tiff', '.tif', '.webp', '.ico')
            videos = ('.mp4', '.mov', '.avi', '.mpg', '.mpeg', '.mkv', '.flv', '.wmv', '.webm')
            audio = ('.mp3', '.wav', '.aac', '.ogg', '.flac', '.m4a', '.wma', '.aiff', '.aif', '.m4r')
            fonts = ('.ttf', '.otf', '.woff', '.woff2', '.eot', '.ttc')
            games = ('.unity3d', '.atlas', '.spine', '.skel', '.ccreator', '.glsl', '.resource', '.bundle')
...
def _extract_strings(app: App): 
    ...
    for plist_file in glob_by_magic(app.get_decompiled_path(), b'bplist'):
        try:
            ...
            logger.warning(f"Failed to convert binary plist file {plist_file} to xml: {e}")
