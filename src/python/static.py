...
def analyze(app: App) -> None:
    ...
    if app.get_stage() < 5:
        logger.error(f"App must be decompiled and indexed before running static analysis")
        return
    
    if app.get_stage() >= 6 and not Config().force:
        logger.info(f"Skipping static analysis of {app.package_id}, results already exist in the working directory")
        return
...
def _analyze_manifest() -> bool:
    ...
    config_res = app_apk.get_android_manifest_xml().find('./application').get(config_key)
    ...
    try:
        ...
        config_file = None
        for _, file in app_apk.get_android_resources().get_resolved_res_configs(config_res):
            config_file = file
            break
        ...
        for detector in Detectors():
            detector.static_analyze_network_security_config(os.path.join(app_apk.filename.replace('.apk', ''), config_file), config_xml, config_dict)
    except Exception:
...
def _analyze_plist() -> None:
    ...
    app = Context().app
    info_plist_file = glob.glob(join(app.get_decompiled_path(), '*', 'Payload', '*.app', 'Info.plist'))[0]
