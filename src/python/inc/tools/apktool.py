...
def decompile(binary: str) -> bool:
    ...
    logger.warn(f'Failed to decompile {binary} with resources, trying without resources')
    cmd = f'java -jar {apktool} d {binary} -o {binary[:-4]} --no-res -f'
    if run_system_command(cmd)[0]:
        return True

    # Try to decompile without resources and without sources
    logger.warn(f'Failed to decompile {binary} without resources, trying without resources and without sources')
    cmd = f'java -jar {apktool} d {binary} -o {binary[:-4]} --no-res --no-src -f'
    if run_system_command(cmd)[0]:
        return True

    logger.warn(f'Failed to decompile {binary} without resources and without sources, trying to unzip')
