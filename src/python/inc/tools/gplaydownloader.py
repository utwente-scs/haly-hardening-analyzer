...
def download_apk(app: App) -> bool:
    ...
    # Download the app
    (success, result, _) = run_system_command(f"java -jar {tools_path('gplay-downloader.jar')} -a {app_ids} -c {auth_config} -o {workdir_path(join('binary', app.os))}")
    if success:
        if not f'Downloaded AppId {app.package_id}' in result:
            ...
            os.rename(join(app.get_binaries_path(), f'{app.package_id}.apk'), join(app.get_binaries_path(), 'base.apk'))
    else:
        logger.error(f"Failed to download {app.package_id}: {result}")
