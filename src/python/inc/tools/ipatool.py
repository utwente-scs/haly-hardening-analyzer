...
def _execute_ipatool_command(command: str, error_ok: bool = False) -> str:
    ...
    if '--auth-code' in result:
        ...
        keychain_passphrase = getpass('Please enter your keychain passphrase to save or access the credentials: ')
        return _execute_ipatool_command(command)
    
    if not success or not '"success":true' in result:
        if not error_ok:
            if keychain_passphrase is not None:
                command = command.replace(keychain_passphrase, '*'*len(keychain_passphrase))
...
def download_ipa(app: App) -> bool:
    ...
    if result is None:
        ...
        result = _execute_ipatool_command(f'auth login -e {Config().app_store_email} -p {Config().app_store_password}')
        if result is None:
    ...
    result = _execute_ipatool_command(f"download --purchase -b {app.package_id} -o {app.get_binaries_path()}")
    if result is None:
        ...
        os.rename(json.loads(result)['output'], join(app.get_binaries_path(), 'base.ipa'))
