...
def kill_process(parent_pid, signal=signal.SIGTERM):
    ...
    for pid in pids[::-1]:
        subprocess.run(
            ["sudo", "kill", f"-{signal.value}", f"{pid}"],
...
class ConnectionDetector(Detector):
    ...
    def static_analyze_network_security_config(
        ...
        self.static_results.append(
            NetworkConfigStaticMessage(config_file, "nsc", config_xml)
        )

        if 'cleartextTrafficPermitted="true"' in config_xml:
            self.static_results.append(
                NetworkConfigStaticMessage(config_file, "plain_http", config_xml)
            )
    ...
    def static_analyze_info_plist(self, plist: dict):
        ...
        if "NSAppTransportSecurity" in plist:
            ...
            for key in arbitrary_loads:
                if (
                    key in plist["NSAppTransportSecurity"]
                    and plist["NSAppTransportSecurity"][key] == True
                ):
                    plist_with_keys = {
                        k: plist["NSAppTransportSecurity"][k]
                        for k in arbitrary_loads
                        if k in plist["NSAppTransportSecurity"]
                    }
                    self.static_results.append(
                        NetworkConfigStaticMessage(
                            Context().app.get_decompiled_path(),
    ...
    def dynamic_before_analysis(self):
        if Context().is_ios():
            logger.info(
                "Waiting before starting analysis of iOS app to minimize background traffic"
            )
