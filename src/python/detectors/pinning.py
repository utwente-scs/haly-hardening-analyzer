...
from models.message import StringStaticMessage, NetworkConfigStaticMessage, SmaliStaticMessage
...
class PinningDetector(Detector):
    ...
    def static_analyze_plaintext(self) -> None:
        ...
        for pattern in patterns:
            for result in search_plaintext(pattern):
                self.static_results.append(StringStaticMessage(result['source'], pattern, result))
            ...
            for file in files:
                ...
                if extension in cert_extensions:
                    with open(os.path.join(root, file), 'rb') as f:
                        ...
                        self.static_results.append(StringStaticMessage(os.path.join(root, file), extension, {
                            'line': line,
                            'line_nr': 1,
                        }))
        ...
        try:
            ...
            for domain in domain_config:
                if 'pin-set' in domain:
                    self.static_results.append(StringStaticMessage(config_file, 'pin-set', {
                        'line': xmltodict.unparse({'pin-set': domain['pin-set']}, full_document=False),
                        'line_nr': 1
                    }))
        except Exception:
            pass
    ...
    def static_analyze_info_plist(self, plist: dict):
        ...
        if 'NSAppTransportSecurity' in plist and 'NSPinnedDomains' in plist['NSAppTransportSecurity'] and len(plist['NSAppTransportSecurity']['NSPinnedDomains']) > 0:
            self.static_results.append(NetworkConfigStaticMessage(Context().app.get_decompiled_path(), 'NSPinnedDomains', plist['NSAppTransportSecurity']['NSPinnedDomains']))
