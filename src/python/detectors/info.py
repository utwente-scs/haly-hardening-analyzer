...
class InfoDetector(Detector):
    ...
    def static_analyze_info_plist(self, plist: dict):
        self.static_results["name"] = (
            plist["CFBundleDisplayName"]
            if "CFBundleDisplayName" in plist
            else plist["CFBundleName"]
        )
        ...
        self.static_results["min_sdk"] = (
            plist["MinimumOSVersion"] if "MinimumOSVersion" in plist else None
        )

        self.static_results["main_activity"] = (
        ...
        self.static_results["permissions"] = {
            permission: plist[permission]
            for permission in ios_permissions
            if permission in plist
        }
    ...
    def static_analyze_r2(self, binary: NativeBinary) -> None:
        if Context().is_ios():
            ...
            if info is not None:
                ...
                self.static_results["entitlements"] = (
                    plistlib.loads(entitlements.encode("utf-8"))
                    if entitlements
                    else None
                )
    ...
    def dynamic_get_data(self) -> dict:
        return {
            "info": {
                "package": (
                    self.static_results["package"]
                    if "package" in self.static_results
                    else Context().app.package_id
                ),
                "executable": (
                    self.static_results["executable"]
                    if "executable" in self.static_results
                    else None
                ),
            }
        }
