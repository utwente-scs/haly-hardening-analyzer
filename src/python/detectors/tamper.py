...
from models.message import SmaliStaticMessage, StringStaticMessage, NativeFunctionStaticMessage
...
class TamperDetector(Detector):
    ...
    def static_analyze_r2(self, binary: NativeBinary) -> None:
        for result in binary.find_objc_call('attestKey:clientDataHash:completionHandler:'):
            self.static_results.append(NativeFunctionStaticMessage(binary.path, 'DCAppAttestService::attestKey', result))
        ...
        if Context().is_android():
            ...
            for smali_file in search_smali(signature):
                for result in smali_file.find_call(signature, 'object'):
                    self.static_results.append(SmaliStaticMessage(smali_file.file, 'android.content.pm.PackageInfo::signatures', result, False))

            # Check for packageInfo.signingInfo
            signature = 'Landroid/content/pm/PackageInfo;->signingInfo:Landroid/content/pm/SigningInfo;'
            for smali_file in search_smali(signature):
                for result in smali_file.find_call(signature, 'object'):
                    self.static_results.append(SmaliStaticMessage(smali_file.file, 'android.content.pm.PackageInfo::signingInfo', result, False))
            ...
            for smali_file in search_smali(signature):
                for result in smali_file.find_call(signature):
                    self.static_results.append(SmaliStaticMessage(smali_file.file, 'android.content.pm.PackageManager::hasSigningCertificate', result))
            ...
            for smali_file in search_smali(signature):
                for result in smali_file.find_call(signature):
                    self.static_results.append(SmaliStaticMessage(smali_file.file, 'com.google.android.play.core.integrity.IntegrityManager::requestIntegrityToken', result))

            # Check for SafetyNet API
            signature = 'Lcom/google/android/gms/safetynet/SafetyNetClient;->attest'
            for smali_file in search_smali(signature):
                for result in smali_file.find_call(signature):
                    self.static_results.append(SmaliStaticMessage(smali_file.file, 'com.google.android.gms.safetynet.SafetyNetClient::attest', result))