...
class KeyloggerDetector(Detector):
    ...
    def static_analyze_r2(self, binary: NativeBinary) -> None:
        ...
        for result in binary.find_objc_call("textInputMode"):
            self.static_results.append(
                NativeFunctionStaticMessage(
                    binary.path, "UIResponder::textInputMode", result
                )
            )
        ...
        for result in binary.find_objc_call("activeInputModes"):
            self.static_results.append(
                NativeFunctionStaticMessage(
                    binary.path, "UITextInputMode::activeInputModes", result
                )
            )
        ...
        for result in binary.find_objc_call("setInputView"):
            self.static_results.append(
                NativeFunctionStaticMessage(binary.path, "UIView::inputView", result)
            )
            ...
            for smali_file in search_smali(signature):
                for result in smali_file.find_call(signature):
                    self.static_results.append(
                        SmaliStaticMessage(
                            smali_file.file,
                            f"android.view.inputmethod.InputMethodManager::{function}",
                            result,
                        )
                    )
        ...
        for smali_file in search_smali(signature):
            for result in smali_file.find_call(signature):
                ...
                self.static_results.append(
                    SmaliStaticMessage(
                        smali_file.file,
                        "android.widget.EditText::setShowSoftInputOnFocus",
                        result,
                        len(result["args"]) > 0 and result["args"][0] is not None,
                    )
                )
