...
class SvcDetector(Detector):
    ...
    def static_analyze_r2(self, binary: NativeBinary):
        svcs = binary.exec_r2_cmd(f'"/aoj svc {Context().get_svc_code()}"') # Search for svc instructions
        ...
        for svc in svcs:
            if not svc['code'].endswith(f'svc {Context().get_svc_code()}'): # "svc 0" also matches "svc 0x..." so we filter it out here
                continue
            ...
            self.static_results.append(SvcStaticMessage(binary.path, syscall_id, syscall, svc['offset'] - offset))

    def dynamic_get_data(self) -> dict:
        return {
            'syscall_names': Context().get_syscall_names()
        }
        ...
        if is_main_process:
            ...
            for svc in svcs:
                ...
                if Context().is_ios():
                    ...
                    path = svc['module'][3:-3] # Remove the "lib" prefix and ".so" suffix
