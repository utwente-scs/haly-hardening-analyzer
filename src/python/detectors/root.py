...
from models.message import StringStaticMessage, NativeFunctionStaticMessage, DynamicMessage
...
class RootDetector(Detector):
    ...
    def _get_data(self) -> dict:
        with open(data_path(f'detectors/root/root-{Context().get_os()}.json5'), 'r') as f:
    ...
    def static_analyze_plaintext(self) -> None:
        ...
        if Context().is_android():
            patterns = [pattern_to_regex(pattern) for pattern in set(patterns + data['apps']['blacklist'])]
        elif Context().is_ios():
            # On iOS, apps are checked by URI
            patterns = [pattern_to_regex(pattern) for pattern in patterns]
            patterns += ['(^|["\'])' + pattern.replace('*', '.*') + '://.*' for pattern in data['apps']['blacklist']]
        ...
        for pattern in patterns:
            for result in search_plaintext(pattern):
                ...
                self.static_results.append(StringStaticMessage(result['source'], pattern, result, confident))
    ...
    def static_analyze_r2(self, binary: NativeBinary) -> None:
        ...
        for syscall in self._get_data()['syscalls']:
            for result in binary.find_syscalls(syscall):
                self.static_results.append(NativeFunctionStaticMessage(binary.path, syscall, result, confident))
