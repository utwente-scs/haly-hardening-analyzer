    ...
    def static_analyze_r2(self, binary: NativeBinary) -> None:
        ...
        for call in functions:
            for result in binary.find_syscalls(call):
                self.static_results.append(NativeFunctionStaticMessage(binary.path, call, result, 'sysctl' not in call))
    ...
    def static_analyze_plaintext(self) -> None:
        ...
        for function in ['isDebuggerConnected', 'waitingForDebugger']:
            ...
            for smali_file in search_smali(signature):
                for result in smali_file.find_call(signature):
                    self.static_results.append(SmaliStaticMessage(smali_file.file, f'android.os.Debug::{function}', result))
        ...
        for smali_file in search_smali(signature):
            for result in smali_file.find_call(signature):
                ...
                self.static_results.append(SmaliStaticMessage(smali_file.file, 'android.content.ContextWrapper::getApplicationInfo', result, False))

        # Check for android.provider.Settings.Secure.getString()
        signatures = ['Landroid/provider/Settings$Secure;->get', 'Landroid/provider/Settings$Global;->get']
            ...
            for smali_file in search_smali(signature):
                for result in smali_file.find_call(signature):
                    ...
                    if len(result['args']) > 1 and result['args'][1] is not None:
                        if result['args'][1] not in ['adb_enabled', 'development_settings_enabled', 'mock_location']:
                            continue
