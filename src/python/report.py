...
def _get_libraries() -> Tuple[list, list]:
    ...
    with open(data_path('libraries/native.txt')) as f:
        known_native_libraries = [lib.strip() for lib in f.read().splitlines() if not lib.startswith('#') and lib.strip() != '']
...
def get_library(result: dict) -> str:
    ...
    java_libs = [lib for lib in java_libs if not lib.startswith(result['app_id']) and not result['app_id'].startswith(lib)]
    ...
    if result['analysis_type'] == 'static':
        ...
        java_backtrace = result['java_backtrace'] if 'java_backtrace' in result else None
        ...
        if result['context'] != 'java':
            ...
            for lib in native_libs:
                if any(lib in item['module'] for item in backtrace if item['module'] is not None):
                    return lib
...
def _get_apps_results() -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, dict]:
    ...
    if not Config().force:
        print("Checking for cached results, use --force to force re-calculating all results")
        cached_data = {}
        for file in ['apps', 'app_infos', 'app_results', 'app_counts']:
            if exists(result_path(f'{file}.csv')):
                data = pd.read_csv(join(result_path(''), f'{file}.csv'), low_memory=False, escapechar='\\')
                if file == 'app_counts':
        ...
        if len(cached_data) == 4:
            ...
            return cached_data['apps'], cached_data['app_infos'], cached_data['app_results'], cached_data['app_counts']
    ...
    for app in tqdm([app for app in Config().apps if app.os == 'android']):
        ...
        for app_os in ['ios', 'android']:
            for analysis_type in ['static', 'dynamic']:
                package_id = android_to_app[app.package_id]['ios_bundle_id'] if app_os == 'ios' else app.package_id
                json_file = join(result_path(app_os), package_id, f'{analysis_type}.json')

                if not exists(json_file):
                    if analysis_type == 'static':
                        analysis_completed = False
                else:
                    app_counts[f'total{app_os.capitalize()}{analysis_type.capitalize()}'] += 1
        ...
        for app_os in ['ios', 'android']:
            for analysis_type in ['static', 'dynamic']:
                app_id = android_to_app[app.package_id]['ios_bundle_id'] if app_os == 'ios' else app.package_id
                other_app_id = app.package_id if app_os == 'ios' else android_to_app[app.package_id]['ios_bundle_id']
                ...
                for detector in result.keys():
                    if detector == 'info':
                        ...
                        for detection_result in result[detector]:
                            ...
                            if detection_result['detector'] == 'tamper' and any(attest in detection_result['function'] for attest in ['IntegrityManager', 'SafetyNetClient']):
...
def _should_ignore_detection(detection_result: dict) -> bool:
    ...
    if detection_result['detector'] == 'hooking':
        lower_line_text = detection_result['line'].lower() if 'line' in detection_result else ''
        fp = {
            'cydia': ['dial'],
            'frida': ['frida khalo', 'sufrida', 'sofrida', 'profile_name', 'alfrida', 'ivett', 'fritiof', 'female', 'feminine', 'first_name', 'giuffrida', 'boy.', 'girl.', 'wilfrid'],
            'xposed': ['axposed'],
        }
        
        if any(pattern in pattern and any(fp in lower_line_text for fp in fp[pattern]) for pattern in fp.keys()):
            return True

        if 'frida' in pattern and (source.endswith('.json') or any(name in source.lower() for name in ['firstname', 'lastname', 'first_name', 'last_name', '/sv/', '_sv', 'sv.lproj'])):
            return True
        ...
        fp = {
            'kinguser': ['networkinguser', 'bookinguser', 'trackinguser', 'cookinguser', 'blockinguser', 'rankinguser', 'parkinguser', 'linkinguser', 'seekinguser', 'talkinguser', 'checkinguser', 'markinguser', 'likinguser', 'speakinguser', 'pickinguser', 'lockinguser', 'talkinguser', 'takinguser', 'bankinguser', '\\u2026kinguser'],
            'bash': ['#!/bin/bash', 'for example'],
            '.su': ['/wiki/.su'],
            'supersu': ['supersub', 'supersuc', 'supersud', 'supersuf', 'supersug', 'supersuk', 'supersul', 'supersum', 'supersun', 'supersup', 'supersur', 'supersus', 'supersut', 'supersuv']
        }
        ...
        if (')su(' in pattern or ')sudo(' in pattern) and not 'bin/' in lower_line_text and 'data/local/' not in lower_line_text:
            return True
        
        if 'magisk' in pattern and [danish in source for danish in ['/da/', '_da', 'da.lproj', 'ideas_info_config.json']]:
            return True
        ...
        if 'genymotion' in pattern and 'running on emulator (or genymotion)' in lower_line_text:
            return True
        
    elif detection_result['detector'] == 'pinning' and detection_result['type'] == 'network_config' and detection_result['config_type'] == 'nsc':
        return True
...
def _get_statistics() -> Tuple[pd.DataFrame, dict]:
        ...
        for _, result in query_results.iterrows():
            if result['detector'] in hardening_techniques:
                ...
                if result['confident'] == 0:
                    key += '?'
                statistics['hardeningTechniques']['values'][key][i_techs.index(result['detector'])] = result['count']
    ...
    if 'hardeningTechniquesApps' in statistics_to_show:
        ...
        for _, result in query_results.iterrows():
            ...
            for x in [result['hardening_count'], min(result['hardening_count'] + 1, n_techs - 1)]:
    ...
    if 'hardeningTechniquesConsistency' in statistics_to_show:
        ...
        query_results = psql('''
            ...
            CROSS JOIN ''' + techniques_inner_query + '''
            LEFT JOIN ''' + ios_inner_query + ''' ON apps.ios_bundle_id = ios_results.app_id AND ios_results.detector = techniques.detector
            LEFT JOIN ''' + android_inner_query + ''' ON apps.android_id = android_results.app_id AND android_results.detector = techniques.detector
        ...
        for _, result in query_results.iterrows():
            ...
            statistics['hardeningTechniquesConsistency']['values'][key][i_techs.index(result['detector'])] = result['app_count']
    ...
    if 'hardeningTechniquesDelta' in statistics_to_show:
        ...
        for _, result in query_results.iterrows():
            statistics['hardeningTechniquesDelta']['values'][int(result['different'])] = int(result['app_count'])
    ...
    ###########################
    categories = psql('''SELECT DISTINCT ios_category FROM apps ORDER BY ios_category''', locals())['ios_category'].tolist()
    ...
    if 'hardeningTechniquesPerCategory' in statistics_to_show:
        ...
        query_results = psql('''
            ...
            LEFT JOIN ''' + ios_inner_query + ''' ON apps.ios_bundle_id = ios_results.app_id
            LEFT JOIN ''' + android_inner_query + ''' ON apps.android_id = android_results.app_id
            GROUP BY apps.ios_category
        ''', locals()).fillna(0)

        for _, result in query_results.iterrows():
            for os in ['android', 'ios']:
                statistics['hardeningTechniquesPerCategory']['values'][os][categories.index(result['ios_category'])] = result[f'detected_{os}']

        # Sort labels, android values and ios values by descending order of the android values
        statistics['hardeningTechniquesPerCategory']['labels'], statistics['hardeningTechniquesPerCategory']['values']['android'], statistics['hardeningTechniquesPerCategory']['values']['ios'] = \
            zip(*sorted(zip(statistics['hardeningTechniquesPerCategory']['labels'], statistics['hardeningTechniquesPerCategory']['values']['android'], statistics['hardeningTechniquesPerCategory']['values']['ios']), key=lambda x: x[1], reverse=True))
    ...
    if 'hardeningTechniquesPerPermission' in statistics_to_show:
        ...
        query_results = psql('''
            ...
            LEFT JOIN ''' + results_inner_query + ''' ON app_infos.app_id = results.app_id AND results.os = app_infos.os
        ...
        for _, result in query_results.iterrows():
            statistics['hardeningTechniquesPerPermission']['values'][result["os"]][list(permissions.keys()).index(result['permission_name'])] = result['techniques_detected']

        # Sort labels, android values and ios values by descending order of the android values
        statistics['hardeningTechniquesPerPermission']['labels'], statistics['hardeningTechniquesPerPermission']['values']['android'], statistics['hardeningTechniquesPerPermission']['values']['ios'] = \
            [list(item) for item in zip(*sorted(zip(statistics['hardeningTechniquesPerPermission']['labels'], statistics['hardeningTechniquesPerPermission']['values']['android'], statistics['hardeningTechniquesPerPermission']['values']['ios']), key=lambda x: x[1], reverse=True))]

        # Move the 'None' category to the end
        index = statistics['hardeningTechniquesPerPermission']['labels'].index('None')
        statistics['hardeningTechniquesPerPermission']['labels'].append(statistics['hardeningTechniquesPerPermission']['labels'].pop(index))
        statistics['hardeningTechniquesPerPermission']['values']['android'].append(statistics['hardeningTechniquesPerPermission']['values']['android'].pop(index))
        statistics['hardeningTechniquesPerPermission']['values']['ios'].append(statistics['hardeningTechniquesPerPermission']['values']['ios'].pop(index))
    ...
    if 'hardeningTechniquesPerPermissionCount' in statistics_to_show:
        ...
        for _, result in query_results.iterrows():
            statistics['hardeningTechniquesPerPermissionCount']['values'][result["os"]][result['permission_count']] = result['techniques_detected']

        if all(statistics['hardeningTechniquesPerPermissionCount']['values'][os][-1] == 0 for os in ['android', 'ios']):
    ...
    if 'hookingFrameworks' in statistics_to_show:
        ...
        query_results = psql('''
            SELECT DISTINCT app_id, os, analysis_type, pattern, file, app FROM app_results
            WHERE (pattern IS NOT NULL OR file IS NOT NULL OR app IS NOT NULL)
        ...
        for _, framework_keywords in hooking_frameworks.items():
            ...
            for _, result in query_results.iterrows():
                if last_app_id == result['app_id'] and last_app_os == result['os'] and last_analysis_type == result['analysis_type']:
                    continue

                if any(any(keyword in result[key].lower() for key in ['pattern', 'file', 'app']) for keyword in framework_keywords):
                    number_of_apps[f'{result["os"]}{result["analysis_type"].capitalize()}'] += 1
                    last_app_id = result['app_id']
            ...
            for os in ['android', 'ios']:
                for analysis_type in ['static', 'dynamic']:
                    ...
                    statistics['hookingFrameworks']['values'][key].append(number_of_apps[key])
    ...
    if 'plaintextTraffic' in statistics_to_show:
        ...
        for _, result in query_results.iterrows():
            statistics['plaintextTraffic']['values'][0 if result['os'] == 'android' else 1] += 1
        ...
        for _, result in query_results.iterrows():
            statistics['plaintextTraffic']['values'][2 if result['os'] == 'android' else 3] += 1
    ...
    if 'plaintextTrafficType' in statistics_to_show:
        ...
        statistics['plaintextTrafficType'] = {
            ...
            'labels': ['OCSP', 'Images', 'Archives', 'Web resources', 'Fonts', 'API', 'Local IP', 'Other'],
        ...
        for _, result in query_results.iterrows():
            ...
            elif any(image_ext + ' HTTP' in request or image_ext + '?' in request for image_ext in ['.png', '.jpg', '.jpeg', '.gif', '.webp', '.bmp', '.svg']):
                statistics['plaintextTrafficType']['values'][1] += 1
            elif any(archive_ext + ' HTTP' in request or archive_ext + '?' in request for archive_ext in ['.zip', '.rar', '.tar', '.tar.gz', '.tar.bz2', '.tar.xz', '.7z', '.tz']):
                statistics['plaintextTrafficType']['values'][2] += 1
            elif any(web_ext + ' HTTP' in request or web_ext + '?' in request for web_ext in ['.js', '.css']):
                statistics['plaintextTrafficType']['values'][3] += 1
            elif any(web_ext + ' HTTP' in request or web_ext + '?' in request for web_ext in ['.otf', '.ttf', '.woff', '.woff2']):
                statistics['plaintextTrafficType']['values'][4] += 1
            elif any(api_query in request for api_query in ['.json HTTP', '.json?', '/v1/', 'api', '.xml HTTP', '.xml?', 'itunes.apple.com']):
                statistics['plaintextTrafficType']['values'][5] += 1
            elif any(local in request for local in ['http://localhost/', 'http://192.168.', 'http://10.']):
    ...
    if 'tlsCipher' in statistics_to_show:
        ...
        statistics['tlsCipher'] = {
            ...
            'values': {
                'android': [count['android'] if 'android' in count else 0 for count in ciphers.values()],
                'ios': [count['ios'] if 'ios' in count else 0 for count in ciphers.values()],
            }
        }
    ...
    if 'hardeningTechniquesLibraries' in statistics_to_show:
        ...
        for _, result in query_results.iterrows():
            ...
            statistics['hardeningTechniquesLibraries']['values'][result['os'] + result['library']][i_techs.index(result['detector'])] = result['app_count']
    ...
    if 'hardeningTechniquesLibrariesNoCommon' in statistics_to_show:
        ...
        for _, result in query_results.iterrows():
            ...
            statistics['hardeningTechniquesLibrariesNoCommon']['values'][result['os'] + result['library']][i_techs.index(result['detector'])] = result['app_count']
    ...
    if 'libraries' in statistics_to_show:
        ...
        for _, result in query_results.iterrows():
            ...
            if result['analysis_type'] not in statistics['libraries'][result['os']][result['detector']]:
                statistics['libraries'][result['os']][result['detector']][result['analysis_type']] = []
            statistics['libraries'][result['os']][result['detector']][result['analysis_type']].append({
                'Library': result['library'],
                'Number of apps': result['app_count']
            })
                    ...
                    for key in data:
                        ...
                        for message in result:
                            for group in grouped_results:
                                if type(message) is type(group[0]) and message.should_group(group[0]):
                                    group.append(message)
                                    break
                            else:
                                grouped_results.append([message])

                        results[atype][os][key] = sorted(grouped_results, key=lambda group: group[0].summary())
